...
const Footer = () => {
  return (
    <footer style={myStyle} className='-mt-[270px] md:-mt-[300px] pt-[300px] '>

      <div className='"w-10/12  pb-10 md:my-0 md:w-11/12 max-w-[1080px] mx-auto flex flex-col lg:flex-row gap-8   
            space-y-6 lg:space-y-0 lg:space-x-4 px-8 '>

        {/* 1st column */}
        <div className='flex flex-col w-full  lg:max-w-[250px] '>
          <img src={razorpayLogo} loading="lazy" width="120px" height="24px" alt={razorpayLogo}/>
          <p className='text-sm text-grayText my-3 leading-[1.625] '>
          ...
          <div className="flex items-start space-x-4">
            <img src={footerCertificate1} loading="lazy" width="92" height="40" className="cursor-pointer"  alt={footerCertificate1} />
            <img src={footerCertificate2} loading="lazy" width="122" height="80" className="cursor-pointer" alt={footerCertificate2} />
          </div>
        ...
        {/* column 2  */}
        <div className="flex flex-col space-y-4 md:space-y-0 sm:flex-row  justify-between">
          {/* sub column - 1 */}
          <div className='grid grid-cols-1 gap-4 p-3'>
            {FooterLinksDataCol_1.map(footerlink => (
              <div key={footerlink.id} className=''>
                <p className='text-md font-bold text-grayBlue'>{footerlink.title}</p>
                <ul className='space-y-1'>
                  {footerlink.Links.map(link1 => (
                    <li key={link1} className='text-base font-normal text-lightBlue hover:text-grayBlue 
          ...
          <div className='grid grid-cols-1 gap-4 p-3 '>
            {FooterLinksDataCol_2.map(footerlink => (
              <div key={footerlink.id} className=''>
                ...
                <ul className='space-y-1'>
                  {footerlink.Links.map(link1 => (
                    <li key={link1} className='text-base font-normal text-lightBlue500 hover:text-grayBlue 
                      transition-all duration-200 cursor-pointer'>
                      {link1}
                    </li>
                  ))}
          ...
          <div className=' grid grid-cols-1 gap-4 p-3 '>
              ...
              <ul className="flex space-x-4 items-center mt-4">
                {socialIcons.map(icon => (
                  <li key={icon.id} className="cursor-pointer">
                    <a href='/'>
                      <img src={icon.img} alt={icon.img} width="24" height="24" loading="lazy" />
                    </a>
                  </li>
                ))}
              </ul>
            ...
            <div>
              ...
              <p className="font-mullish text-sm whitespace-nowrap">
